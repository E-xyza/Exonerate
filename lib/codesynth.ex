defmodule Exonerate.Codesynth do
  ...
  def validatorfn_string(name, false),
    do:
      "def validate_#{name}(val), do: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}"
  ...
  def validatorfn_string(name, schema = %{"multipleOf" => v, "type" => "integer"}) do
    "def validate_#{name}(val) when is_integer(val) and (rem(val,#{v}) != 0), do: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}\n" <>
  ...
  def validatorfn_string(
    ...
    "def validate_#{name}(val) when is_integer(val) and val <= #{v}, do: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}\n" <>
  ...
  def validatorfn_string(
    ...
    "def validate_#{name}(val) when is_number(val) and val <= #{v}, do: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}\n" <>
      validatorfn_string(name, Map.delete(schema, "minimum"))
  end
...
imum"))
  ...
  def validatorfn_string(name, schema = %{"minItems" => l}) do
    "def validate_#{name}(val) when is_list(val) and length(val) < #{l}, do: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}\n" <>
  ...
  def validatorfn_string(name, schema = %{"required" => _}, type = "object") do
    """
      ...
      def validate_#{name}(val) when is_map(val), do: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}
    """
  end
  ...
  def validate_qualifier_string(name, schema = %{"anyOf" => list}) do
    ...
    """
      def validate_#{name}__anyof(val) do
        cond do
          #{validation_list}
          true -> {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}
        end
      end
  ...
  def validate_qualifier_string(name, schema = %{"oneOf" => list}) do
    ...
    """
      def validate_#{name}__oneof(val) do
        ...
        if count == 1, do: :ok, else: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}
      end
  ...
  def validate_qualifier_string(name, schema = %{"enum" => list}) do
    ...
    """
      def validate_#{name}__enum(val) do
        if val in [#{validation_list}], do: :ok, else: {:error, \"\#{Jason.encode! val} does not conform to JSON schema\"}
      end
