defmodule ExonerateTest.Automate do
  ...
  defp to_describe_block({%{"description" => description!, "schema" => schema!, "tests" => tests}, index}, path, opts) do
    ...
    test_blocks = tests
    |> Enum.with_index
    |> Enum.reject(fn {_test, inner_index} -> {path, index, inner_index} in opts[:omit_tests] end)
    |> Enum.map(&(elem(&1, 0)))
