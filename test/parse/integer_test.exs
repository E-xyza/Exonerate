  ...
  describe "range parameters are set" do
    test "maximum" do
      assert %{
             ...
             } = Integer.parse(@context, %{"maximum" => 3})
    end

    test "minimum" do
      assert %{
               filters: [%Minimum{minimum: 3}]
             } = Integer.parse(@context, %{"minimum" => 3})
    end

    test "exclusiveMaximum" do
      assert %{
               filters: [%ExclusiveMaximum{maximum: 3}]
             } = Integer.parse(@context, %{"exclusiveMaximum" => 3})
    end

    test "exclusiveMinimum" do
      assert %{
               filters: [%ExclusiveMinimum{minimum: 3}]
             } = Integer.parse(@context, %{"exclusiveMinimum" => 3})
    end
  end
  ...
  describe "multipleOf parameters is set" do
    test "correctly" do
      assert %{
             ...
             } = Integer.parse(@context, %{"multipleOf" => 3})
    end
  end
end
